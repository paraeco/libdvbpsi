...
{
  dvbpsi_ssu_target_mac_address_dr_t * p_decoded;
  uint8_t *p_data;
  uint8_t length;
  ...
  /* Allocate memory */
  p_decoded = (dvbpsi_ssu_target_mac_address_dr_t *)malloc(sizeof(dvbpsi_ssu_target_mac_address_dr_t));
  ...
  p_decoded->i_mac_addr_mask[0] = p_data[0];
  p_decoded->i_mac_addr_mask[1] = p_data[1];
  p_decoded->i_mac_addr_mask[2] = p_data[2];
  p_decoded->i_mac_addr_mask[3] = p_data[3];
  p_decoded->i_mac_addr_mask[4] = p_data[4];
  p_decoded->i_mac_addr_mask[5] = p_data[5];
  p_data += 6;
  length -= 6;
  
  p_decoded->i_mac_addr_match_length = length;
  if (length)
	memcpy (p_decoded->p_mac_addr_match, p_data, length);  
